...
CAP is a two-tier Daml interface library for mechanisms that collect (private)
submissions, resolve them into an outcome, and execute that outcome with
pre-committed authority. **cap-core** fixes the shape every such mechanism
shares; a domain standard (**cap-governance** here, **cap-auctions** next)
...
This document shows the design the way a reviewer needs it: the interfaces,
the proof they are reusable, their fit with Splice, the trade-offs, and what
is enforced versus trusted. Terms link to the [glossary](GLOSSARY.md); the
file-by-file reference is [`DESIGN.md`](DESIGN.md).
...
| `Submittable` | One live [submission](GLOSSARY.md#submission) [slot](GLOSSARY.md#slot), bound to its mechanism by contract group | — | [`SubmittableV1.daml`](cap-core/Interfaces/submittable/daml/Cap/Core/SubmittableV1.daml) |
...
| `GovernanceOutcome` | The approved action; carries governance's single execute choice | `GovernanceOutcome_Execute` | [`OutcomeV1.daml`](cap-governance/Interfaces/outcome/daml/Cap/Governance/OutcomeV1.daml) |

The lifecycle passes through four fixed choices: [cast](GLOSSARY.md#cast) →
[resolve](GLOSSARY.md#resolution) → execute → [expire](GLOSSARY.md#expiry). Each
choice's checks run in its fixed body; the implementation supplies the format —
its `tally`, eligibility, effect ([`DESIGN.md` §5](DESIGN.md#5-the-lifecycle-end-to-end)).

## 2. The proof of reusability: Splice's DSO governance, rebuilt on CAP

The committee asked for the proof point that the primitives are reusable, and
suggested one: show that SV governance can be expressed with these libraries.
That is the Milestone 1 prototype, [`examples/BabyDso`](examples/BabyDso/DEMOS.md).

Splice's DSO runs three approval mechanisms of deliberately different shapes —
...
same mechanisms on the CAP interfaces. Every demo runs in both packages with
...
Generality is shown in both directions: the three Splice shapes fit the
interfaces, and the interfaces also carry a format Splice does not have (the
container [ballot](GLOSSARY.md#ballot) format, demo 0) and a scenario outside
Splice's design scope (two organizations under one authority party, demo 4).
The catalogue of what each demo proves is
[`DEMOS.md`](examples/BabyDso/DEMOS.md); how to run them and the expected
output is [`README.md#running-the-demos`](README.md#running-the-demos).

## 3. Fit with Splice governance

Splice hand-writes each of its three mechanisms end to end: window,
eligibility, threshold, execution are re-implemented per mechanism inside
`DsoRules`. CAP
factors the shared structure into fixed bodies, so a mechanism definition
shrinks to its format: a `tally`, an eligibility source, an effect
([one `ActionSpec` record per action](examples/BabyDso/DEMOS.md#the-actions)).

The code reuse is visible in the diff between the two packages. `original/`
defines for itself what `cap-version/` imports: the checked-fetch [admission](GLOSSARY.md#admission)
[tools](GLOSSARY.md#tool), `require`, the `Patchable` [merge](GLOSSARY.md#merge), the median. Per mechanism, the
lifecycle checks — window and eligibility, ballot [admission](GLOSSARY.md#admission), [verdict](GLOSSARY.md#verdict)
handling, execution at most once — come from the fixed bodies of
...
(`quorateMajorityAll`). And the reuse compounds *inside* Splice: all three
mechanisms share the same cast, withdraw, and admission code, where the plain
version carries a separate copy per mechanism.
...
- **Decisions stay inside their institution.** In the plain reproduction,
  org1's approved action *executes on org2's state* when one party hosts two
...
- **Concurrent approvals are handled, not raced.** State-[pinned](GLOSSARY.md#pin)
  actions detect [drift](GLOSSARY.md#drift) between approval and execution and
  route it to a declared policy — abort, merge, or a bounded tolerance
  (demos 1–2). Splice's plain flows have no equivalent; last write wins.
...
The point of the rebuild is that Splice *could* reuse it, mechanism by
mechanism, without changing observable behaviour.


## 4. Interface design decisions and trade-offs
...
decision lives in code: [`DESIGN.md` §6](DESIGN.md#6-where-each-design-decision-lives).)
...
| The submission **window is on the `MechanismView`** — mandatory open, optional close — and the timing hooks can **only tighten** it | Timing left wholly to implementations — no cross-implementation guarantee survives; a mandatory close — locks out [standing](GLOSSARY.md#standing) mechanisms with no fixed close |
| cap-core's `Outcome` **declares no execute choice** — identity, window, [expiry](GLOSSARY.md#expiry) only; the execute choice is each domain's own, layered via `requires` | One generic execute choice in the core — forces a single execution shape (arguments, target model) on every domain forever |
...
## 5. What is enforced, and what is trusted

Fixed bodies **enforce**, on-ledger, at every resolution and execution:

- the submission **window** at cast and the floor at resolve;
- **eligibility** against `voters` when the format declares it;
- the **authority signature** on the mechanism and on every presented
...
| executor — execute on state the approvers did not see | state pin + drift policy | demos 1, 2 |
...
own: the `actors : [Party]` pattern on execute/settle choices, `ExtraArgs` /
...
`Splice.Util`, re-exported. cap-auctions — interfaces already written
([`cap-auctions/`](cap-auctions/)) — composes settlement directly with Token
